...
// ------------------------------------for MAC OS CLANG & Windows VS2010 COMPILER NEED TO REDEFINE MACROS:
    #ifdef qDebug
    #undef qDebug
    #endif
 ...
 * \endcode
 *
 * Logger internally uses the lazy-initialised singleton object and needs no definite initialization, but you may
        ...
        Logger
{
  public:
    ...
    //! set filter level for logs:
    static LogLevel filterLevel(LogLevel level = None) {

        static LogLevel filterLogLevel = None;
